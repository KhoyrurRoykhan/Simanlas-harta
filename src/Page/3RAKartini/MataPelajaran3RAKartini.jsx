// src/pages/MataPelajaran3RAKartini.jsx
...
// Fungsi untuk ekstrak File ID dari Google Drive
...
// Fungsi untuk cek apakah link adalah Google Drive
...
const MataPelajaran3RAKartini = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (querySnapshot.empty) {
          setError('Belum ada mata pelajaran untuk Kelas 3 RA Kartini');
          setIsLoading(false);
          return;
        }
        ...
        const processedSubjects = mapelList.map((item) => {
          ...
          const namaKelas = item.namaKelas || 'Kelas 3 RA Kartini';
          ...
          return {
            ...
            path: `/3-ra-kartini/mata-pelajaran/${item.nama?.toLowerCase().replace(/\s+/g, '-') || item.id}`,
            pertemuan: pertemuan,
            // Simpan data kelas
            namaKelas: namaKelas,
            kelas: item.kelas
        ...
        if (processedSubjects.length > 0) {
          setKelasInfo({
            nama: processedSubjects[0].namaKelas || 'Kelas 3 RA Kartini',
            kode: processedSubjects[0].kelas || '3_ra_kartini'
          });
        }
  ...
  const openPreview = (title, link, type) => {
    ...
    const previewLink = isGoogleDriveLink(link) ? convertDriveLink(link, 'preview') : link;
  ...
  const renderPreview = () => {
    ...
    const { title, link, originalLink, type, isDrive } = previewData;
    const viewLink = isDrive ? convertDriveLink(originalLink, 'view') : originalLink;
          ...
          <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-gray-50">
            <div>
              ...
              <p className="text-xs text-gray-500">
                ...
                {isDrive && (
                  <span className="ml-2 text-green-600">• Google Drive</span>
                )}
              </p>
            </div>
                ...
                {isSelected && (
                  <div className="mt-4 pt-4 border-t border-gray-200 space-y-3 animate-fadeIn">
                    ...
                    <div className="flex items-start space-x-2 text-sm text-gray-600">
                      ...
                      <span>Guru: {subject.teacher}</span>
                    </div>
                    <div>
