    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (querySnapshot.empty) {
          setError('Data Matematika untuk Kelas 3 Dewi Sartika tidak ditemukan');
          setIsLoading(false);
          return;
        }
        ...
        // DEBUG: Log data pertemuan
        console.log('📝 Data pertemuan dari database:', pertemuanList);
        pertemuanList.forEach((p, index) => {
          console.log(`📝 Pertemuan ${index + 1}:`, {
            judul: p.judul,
            'bahanAjar (lowercase)': p.bahanAjar,
            'lKpd (with capital K)': p.lKpd,
            'lkpd (lowercase)': p.lkpd,
            'Lkpd (capital L)': p.Lkpd
          });
        });
        ...
        setData({
          mapel: {
            ...
            deskripsi: mapelData.deskripsi || 'Mempelajari konsep bilangan, operasi hitung, geometri, dan pengukuran',
            icon: mapelData.icon || '📐',
    ...
    return (
      <div className="space-y-4">
        {pertemuanList.map((pertemuan) => {
          // CEK SEMUA KEMUNGKINAN FIELD UNTUK LKPD
          const bahanAjarValue = pertemuan.bahanAjar || '';
          // Cek berbagai kemungkinan penamaan field LKPD
          ...
          console.log(`📝 Render pertemuan: ${pertemuan.judul}`);
          console.log(`  - bahanAjar: "${bahanAjarValue}" (${hasBahanAjar ? 'ADA' : 'KOSONG'})`);
          console.log(`  - lKpd: "${lkpdValue}" (${hasLkpd ? 'ADA' : 'KOSONG'})`);
          ...
          return (
            <div key={pertemuan.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
              ...
              {/* Content Pertemuan - Detail saat accordion dibuka */}
              {expandedItems[`pertemuan-${pertemuan.id}`] && (
